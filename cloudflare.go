...
func New(ctx context.Context, next http.Handler, config *Config, name string) (http.Handler, error) {
	...
	if len(config.TrustedCIDRs) > 0 {
		...
		for _, trustedCIDR := range config.TrustedCIDRs {
	...
	for _, allowedCIDR := range config.AllowedCIDRs {
...
func (c *Cloudflare) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for i := 0; i < len(ipList); i++ {
		...
		if sip == nil {
			if c.debug {
				log.Printf("debug: bad ip %s", ipList[i])
			}
		...
		trustIp, checkIPerr := c.trustedChecker.CheckIP(r.Context(), sip)
		if checkIPerr != nil {
			if c.debug {
				log.Println(fmt.Errorf("debug: %w", checkIPerr))
			}
			if c.continueOnError {
				if c.debug {
					log.Printf("debug: continue on error for ip %s\n", ipList[i])
				}
				continue
			}
		...
		allowIp, checkIPerr := c.allowedChecker.CheckIP(r.Context(), sip)
		if checkIPerr != nil && c.continueOnError {
			if c.debug {
				log.Printf("debug: continue on error for ip %s\n", ipList[i])
			}
			continue
	...
	if !allowed {
		if c.debug {
			log.Printf("debug: deny request from: %s\n", strings.Join(ipList, ","))
		}
