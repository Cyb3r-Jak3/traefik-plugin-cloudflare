...
import (
	...
	cloudflare "github.com/Cyb3r-Jak3/traefik-plugin-cloudflare"
	"github.com/Cyb3r-Jak3/traefik-plugin-cloudflare/internal"
)
...
func TestCloudflare(t *testing.T) {
	...
	t.Run("automatic CIDRs", func(t *testing.T) {
		...
		t.Run("disallowed ipv4", func(t *testing.T) {
			...
			res := rr.Result()
			requireEqual(t, http.StatusForbidden, res.StatusCode)
		})
		...
		t.Run("allowed ipv6", func(t *testing.T) {
			...
			res := rr.Result()
			requireEqual(t, http.StatusOK, res.StatusCode)
		})
	...
	t.Run("automatic CIDRs periodic update", func(t *testing.T) {
		...
		t.Run("expired with empty cidrs", func(t *testing.T) {
			...
			res1 := rr1.Result()
			requireEqual(t, http.StatusForbidden, res1.StatusCode)
			...
			res2 := rr2.Result()
			requireEqual(t, http.StatusForbidden, res2.StatusCode)
		})
	...
	t.Run("overwrite header request", func(t *testing.T) {
		...
		t.Run("overwrite", func(t *testing.T) {
			...
			xff := strings.Join(req.Header.Values("X-Forwarded-For"), ",")
			requireEqual(t, "1.2.3.4, 2.2.2.2", xff)
		...
		t.Run("missing header", func(t *testing.T) {
			...
			res := rr.Result()
			requireEqual(t, http.StatusBadRequest, res.StatusCode)

			body, err := io.ReadAll(res.Body)
			if err != nil {
				t.Error(err)
			}
			requireEqual(t, "cf:Bad Request\n", string(body))
		})
	})
	...
	t.Run("overwrite header request with append xff", func(t *testing.T) {
		...
		t.Run("overwrite", func(t *testing.T) {
			...
			xff := strings.Join(req.Header.Values("X-Forwarded-For"), ",")
			requireEqual(t, "1.2.3.4, 1.1.1.1, 2.2.2.2", xff)
	...
	t.Run("no overwrite header request", func(t *testing.T) {
		...
		t.Run("valid", func(t *testing.T) {
			...
			xff := strings.Join(req.Header.Values("X-Forwarded-For"), ",")
			requireEqual(t, "2.2.2.2, 3.3.3.3", xff)

			xri := strings.Join(req.Header.Values("X-Real-Ip"), ",")
			requireEqual(t, "10.10.10.10", xri)

			xfp := strings.Join(req.Header.Values("X-Forwarded-Proto"), ",")
			requireEqual(t, "http", xfp)
